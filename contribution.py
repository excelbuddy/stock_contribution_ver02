...
# ── HIGHLIGHT STATE ───────────────────────────────────────────────────────────
# Dung st.session_state["selected_stock"] de luu ma CP dang duoc highlight.
# Click vao CP nao -> set session_state -> rerun -> tat ca bang to vang dong do.

HIGHLIGHT_COLOR = "#7b6800"   # nen vang dam (dark mode)
HIGHLIGHT_TEXT  = "#ffe066"   # chu vang sang

def _init_state():
    if "selected_stock" not in st.session_state:
        st.session_state["selected_stock"] = ""

def _stock_button_js():
    """Inject JS + CSS de xu ly click CP va trigger rerun qua query param."""
    return """
<script>
function selectStock(code) {
    // Gui gia tri len Streamlit qua window.parent postMessage
    const data = {type: "streamlit:setComponentValue", value: code};
    // Fallback: dung URL param de rerun
    const url = new URL(window.location.href);
    url.searchParams.set("stock", code);
    window.history.pushState({}, "", url);
    // Trigger Streamlit rerun bang cach thay doi hash
    window.location.hash = "stock=" + code + "_" + Date.now();
}
</script>
"""
...
def _build_header(num, ptype, sd, ed, days, vi_s, vi_e, gap_pts, chg):
    ...
    p.append('<div style="display:flex;justify-content:space-between;'
             'align-items:center;">')
    p.append('<span style="font-size:11px;color:#ccc;">Dinh/Day '
             + str(num) + '</span>')
    ...
    p.append('<div style="font-size:11px;color:#eee;margin-top:4px;">'
             + sd.strftime("%Y-%m-%d") + ' &rarr; '
             + ed.strftime("%Y-%m-%d") + '</div>')
...
def _build_table(gainers, losers, gap_pts, selected):
    """
    Build HTML table. Rows matching `selected` get yellow highlight.
    Each CP cell is clickable -> sets ?stock=CODE in URL -> Streamlit rerun.
    """
    p = []
    ...
    for i in range(max_rows):
        ...
        if i < len(gainers):
            ...
            g_pct  = "{:.1f}%".format(
                g["InfluenceIndex"] / gap_pts * 100) if gap_pts else ""
        ...
        if i < len(losers):
            ...
            l_pct  = "{:.1f}%".format(
                l["InfluenceIndex"] / gap_pts * 100) if gap_pts else ""
        ...
        # Tach tung cell de co the highlight rieng biet
        g_bg = HIGHLIGHT_COLOR if g_hl else row_bg
        l_bg = HIGHLIGHT_COLOR if l_hl else row_bg
        g_tc = HIGHLIGHT_TEXT  if g_hl else "#00e676"
        l_tc = HIGHLIGHT_TEXT  if l_hl else "#ff5252"

        # onclick: set URL param va rerun streamlit
        g_click = 'onclick="(function(){' \
                  'const u=new URL(window.parent.location.href);' \
                  'u.searchParams.set(\'stock\',\'' + g_code + '\');' \
                  'window.parent.history.pushState({},\'\',u);' \
                  'window.parent.location.reload();})();" ' \
                  'style="cursor:pointer;"' if g_code else ""
        l_click = 'onclick="(function(){' \
                  'const u=new URL(window.parent.location.href);' \
                  'u.searchParams.set(\'stock\',\'' + l_code + '\');' \
                  'window.parent.history.pushState({},\'\',u);' \
                  'window.parent.location.reload();})();" ' \
                  'style="cursor:pointer;"' if l_code else ""

        p.append('<tr>')
        # Gainers cols
        p.append('<td ' + g_click + ' style="background:' + g_bg
                 + ';color:' + g_tc + ';font-weight:700;padding:3px 6px;'
                 + ('border-left:3px solid ' + HIGHLIGHT_TEXT + ';' if g_hl else '')
                 + '">' + g_code + '</td>')
        p.append('<td style="background:' + g_bg + ';color:' + g_tc
                 + ';text-align:right;padding:3px 6px;">' + g_inf + '</td>')
        p.append('<td style="background:' + g_bg + ';color:' + g_tc
                 + ';text-align:right;padding:3px 6px;font-size:11px;">'
                 + g_pct + '</td>')
        # Losers cols
        p.append('<td ' + l_click + ' style="background:' + l_bg
                 + ';color:' + l_tc + ';font-weight:700;padding:3px 6px;'
                 + ('border-left:3px solid ' + HIGHLIGHT_TEXT + ';' if l_hl else '')
                 + '">' + l_code + '</td>')
        p.append('<td style="background:' + l_bg + ';color:' + l_tc
...
def render_tab1(pc, combined):
    ...
    # Doc stock tu URL query param (Streamlit >= 1.30 ho tro st.query_params)
    try:
        qp = st.query_params
        url_stock = qp.get("stock", "")
        if isinstance(url_stock, list):
            url_stock = url_stock[0] if url_stock else ""
        if url_stock and url_stock != st.session_state["selected_stock"]:
    ...
    with col_f4:
        # Hien thi CP dang duoc chon + nut xoa chon
        if selected:
            st.markdown(
                '<div style="margin-top:28px;padding:4px 10px;'
                'background:#7b6800;border-radius:6px;display:inline-block;'
                'color:#ffe066;font-weight:700;font-size:13px;">'
                'Dang chon: ' + selected + '</div>',
                unsafe_allow_html=True)
            if st.button("X Xoa chon", key="clear_stock"):
                st.session_state["selected_stock"] = ""
                try:
            ...
            st.markdown(
                ...
                'Click vao ma CP de highlight</div>',
                unsafe_allow_html=True)
    ...
    for row_start in range(0, len(periods), cols_per_row):
        ...
        for col_idx, period in enumerate(row_periods):
            ...
            mask = ((combined["Start Date"] == sd)
                    & (combined["End Date"] == ed))
            df_p = combined[mask].copy()

            gainers = df_p[df_p["Type"] == "Gainers"].nlargest(
                top_n, "InfluenceIndex")
            losers  = df_p[df_p["Type"] == "Losers"].nsmallest(
                top_n, "InfluenceIndex")
...
def render_tab2(hist_df, pc):
    ...
    with c1:
        year_from = st.number_input(
            "Tu nam", min_value=2000, max_value=2030, value=2015, step=1)
    with c2:
        year_to = st.number_input(
            "Den nam", min_value=2000, max_value=2030, value=2026, step=1)
    with c3:
    ...
    if show_ann:
        ...
        for i in range(len(pts) - 1):
            p0, p1 = pts[i], pts[i + 1]
            if (p0["date"].year < year_from
                    or p1["date"].year > year_to + 1):
                continue
    ...
    def color_chg(v):
        try:
            ...
            return ("color:#00e676;font-weight:bold" if n > 0
...
def render_tab3(pc, combined):
    ...
    with col_a:
        ...
        if not gainers_data.empty:
            g_agg = (gainers_data.groupby("StockCode")
                     ...
                     .reset_index())
            g_agg = g_agg[g_agg["count"] >= min_periods].nlargest(
                top_stocks, "total")
            fig_g = go.Figure(go.Bar(
                x=g_agg["total"], y=g_agg["StockCode"],
                orientation="h", marker_color="#00c853",
                customdata=np.stack(
                    [g_agg["count"], g_agg["avg"]], axis=-1),
                hovertemplate=(
                    "<b>%{y}</b><br>Tong: %{x:.1f}"
                    "<br>So dot: %{customdata[0]}"
                    "<br>TB/dot: %{customdata[1]:.2f}<extra></extra>"),
            ))
            fig_g.update_layout(
        ...
        if not losers_data.empty:
            l_agg = (losers_data.groupby("StockCode")
                     .agg(total=("InfluenceIndex", "sum"),
                          ...
                          avg=("InfluenceIndex", "mean"))
                     .reset_index())
            l_agg = l_agg[l_agg["count"] >= min_periods].nsmallest(
                top_stocks, "total")
            fig_l = go.Figure(go.Bar(
                x=l_agg["total"].abs(), y=l_agg["StockCode"],
                orientation="h", marker_color="#ff1744",
                customdata=np.stack(
                    [l_agg["count"], l_agg["avg"].abs()], axis=-1),
                hovertemplate=(
                    "<b>%{y}</b><br>Tong: -%{x:.1f}"
                    "<br>So dot: %{customdata[0]}"
                    "<br>TB/dot: %{customdata[1]:.2f}<extra></extra>"),
    ...
    if not pivot.empty:
        fig_h = go.Figure(go.Heatmap(
            ...
            colorscale=[[0.0, "#b71c1c"], [0.4, "#880e4f"],
                        [0.5, "#1a1a2e"], [0.6, "#1b5e20"],
                        [1.0, "#00e676"]],
            zmid=0,
            text=np.round(pivot.values, 1),
            texttemplate="%{text}", textfont=dict(size=9),
            hovertemplate=(
                "<b>%{y}</b> | %{x}<br>Diem: %{z:.2f}<extra></extra>"),
    ...
    up_app = (all_data[(all_data["period_type"] == "UP")
                       & (all_data["InfluenceIndex"] > 0)]
              .groupby("StockCode")["InfluenceIndex"]
              .agg(["count", "sum", "mean"])
              .rename(columns={"count": "uc", "sum": "ut", "mean": "ua"}))
    dn_app = (all_data[(all_data["period_type"] == "DOWN")
                       & (all_data["InfluenceIndex"] < 0)]
              .groupby("StockCode")["InfluenceIndex"]
              .agg(["count", "sum", "mean"])
    ...
    dp = dp[dp[["So dot UP tang", "So dot DOWN giam"]].max(axis=1)
            >= min_periods]
    ...
    def cc(v):
        try:
            return "color:#00e676" if float(v) > 0 else "color:#ff5252"
        except:
            return ""
